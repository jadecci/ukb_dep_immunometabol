...
def corr_analysis(
    ...
    for gender_i in [0, 1]:
        for diagn in [0, 1]:
            data_curr = data.loc[
                (data["31-0.0"] == gender_i) & (data["MDD diagnosis"] == diagn)]
            for pheno_col in pheno_cols[pheno]:
                for dep_col in field_dict["Dep score"]:
                    ...
                    data_corr[ind] = {
                        ...
                        "Field description": pheno_desc_curr, "MDD diagnosis": diagn}
...
field_dict = {"Dep score": ["Sum score (cluster 5)", "Sum score (cluster 6)"], "Diagn ICD10": []}
col_dtypes = {
